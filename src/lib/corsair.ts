/**
 * Corsair API client
 * Corsair provides building blocks for Gmail and Google Calendar integrations,
 * webhooks, and MCP-powered agent access.
 */

const CORSAIR_BASE_URL = process.env.CORSAIR_BASE_URL || 'https://api.corsair.dev'
const CORSAIR_API_KEY = process.env.CORSAIR_API_KEY!

async function corsairFetch(path: string, options: RequestInit = {}) {
  const res = await fetch(`${CORSAIR_BASE_URL}${path}`, {
    ...options,
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${CORSAIR_API_KEY}`,
      ...(options.headers || {}),
    },
  })
  if (!res.ok) {
    const text = await res.text()
    throw new Error(`Corsair API error ${res.status}: ${text}`)
  }
  return res.json()
}

// ─── Gmail via Corsair ───────────────────────────────────────────────────────

export async function corsairListEmails(
  accessToken: string,
  params: {
    maxResults?: number
    pageToken?: string
    q?: string
    labelIds?: string[]
  } = {}
) {
  return corsairFetch('/gmail/messages/list', {
    method: 'POST',
    body: JSON.stringify({ accessToken, ...params }),
  })
}

export async function corsairGetEmail(accessToken: string, messageId: string) {
  return corsairFetch('/gmail/messages/get', {
    method: 'POST',
    body: JSON.stringify({ accessToken, messageId }),
  })
}

export async function corsairSendEmail(
  accessToken: string,
  params: {
    to: string
    subject: string
    body: string
    cc?: string
    bcc?: string
    replyToMessageId?: string
    threadId?: string
  }
) {
  return corsairFetch('/gmail/messages/send', {
    method: 'POST',
    body: JSON.stringify({ accessToken, ...params }),
  })
}

export async function corsairModifyEmail(
  accessToken: string,
  messageId: string,
  addLabels: string[],
  removeLabels: string[]
) {
  return corsairFetch('/gmail/messages/modify', {
    method: 'POST',
    body: JSON.stringify({ accessToken, messageId, addLabels, removeLabels }),
  })
}

export async function corsairSearchEmails(accessToken: string, query: string, maxResults = 20) {
  return corsairFetch('/gmail/search', {
    method: 'POST',
    body: JSON.stringify({ accessToken, query, maxResults }),
  })
}

export async function corsairGetThread(accessToken: string, threadId: string) {
  return corsairFetch('/gmail/threads/get', {
    method: 'POST',
    body: JSON.stringify({ accessToken, threadId }),
  })
}

// ─── Google Calendar via Corsair ─────────────────────────────────────────────

export async function corsairListEvents(
  accessToken: string,
  params: {
    timeMin?: string
    timeMax?: string
    maxResults?: number
    calendarId?: string
  } = {}
) {
  return corsairFetch('/calendar/events/list', {
    method: 'POST',
    body: JSON.stringify({ accessToken, calendarId: 'primary', ...params }),
  })
}

export async function corsairCreateEvent(
  accessToken: string,
  params: {
    title: string
    description?: string
    location?: string
    startTime: string
    endTime: string
    attendees?: string[]
    addGoogleMeet?: boolean
    calendarId?: string
  }
) {
  return corsairFetch('/calendar/events/create', {
    method: 'POST',
    body: JSON.stringify({ accessToken, calendarId: 'primary', ...params }),
  })
}

export async function corsairUpdateEvent(
  accessToken: string,
  eventId: string,
  params: {
    title?: string
    description?: string
    startTime?: string
    endTime?: string
    attendees?: string[]
    calendarId?: string
  }
) {
  return corsairFetch('/calendar/events/update', {
    method: 'POST',
    body: JSON.stringify({ accessToken, eventId, calendarId: 'primary', ...params }),
  })
}

export async function corsairDeleteEvent(
  accessToken: string,
  eventId: string,
  calendarId = 'primary'
) {
  return corsairFetch('/calendar/events/delete', {
    method: 'POST',
    body: JSON.stringify({ accessToken, eventId, calendarId }),
  })
}

// ─── Corsair Webhooks ────────────────────────────────────────────────────────

export async function corsairRegisterWebhook(params: {
  type: 'gmail' | 'calendar'
  accessToken: string
  webhookUrl: string
  userId: string
}) {
  return corsairFetch('/webhooks/register', {
    method: 'POST',
    body: JSON.stringify(params),
  })
}

// ─── Corsair MCP Agent ───────────────────────────────────────────────────────

export async function corsairAgentChat(params: {
  message: string
  accessToken: string
  userId: string
  history?: Array<{ role: string; content: string }>
}) {
  return corsairFetch('/agent/chat', {
    method: 'POST',
    body: JSON.stringify(params),
  })
}
